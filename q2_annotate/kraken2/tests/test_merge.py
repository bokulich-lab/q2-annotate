...
class TestTreeMerging(TestPluginBase):
    ...
    def _df_conv(self, df):
        # this retains the dtype of the resultant DF when sorting
        df["perc_frags_covered"] = df["perc_frags_covered"].astype(float).round(2)
    ...
    def test_merge_trees_no_unclassified_nodes(self):
        ...
        """
        tree1 = self._get_tree("no-unclassified/tree-1.report.txt")
        tree2 = self._get_tree("no-unclassified/tree-2.report.txt")
        obs_df = _dump_tree_to_report(*_merge_trees(tree1, tree2))

        exp_df = self._get_tree_df("no-unclassified/merged-tree.report.txt")

        assert_frame_equal(self._df_conv(obs_df), self._df_conv(exp_df))
    ...
    def test_merge_trees_one_unclassified_nodes(self):
        ...
        """
        tree1 = self._get_tree("one-unclassified/tree-1.report.txt")
        tree2 = self._get_tree("one-unclassified/tree-2.report.txt")
        obs_df = _dump_tree_to_report(*_merge_trees(tree1, tree2))
    ...
    def test_merge_trees_two_unclassified_nodes(self):
        ...
        """
        tree1 = self._get_tree("two-unclassified/tree-1.report.txt")
        tree2 = self._get_tree("two-unclassified/tree-2.report.txt")
        obs_df = _dump_tree_to_report(*_merge_trees(tree1, tree2))
...
class TestResultMerging(TestPluginBase):
    ...
    def _assert_reports_equal(
        ...
        first_df = (
            first.view(pd.DataFrame)
            .sort_values(by="perc_frags_covered")
            .reset_index(drop=True)
        )
        second_df = (
            second.view(pd.DataFrame)
            .sort_values(by="perc_frags_covered")
            .reset_index(drop=True)
        )
    ...
    def _assert_directory_formats_equal(self, first, second, type: str, mags=False):
        first_file_dict = first.file_dict()
        second_file_dict = second.file_dict()

        self.assertEqual(list(first_file_dict.keys()), list(second_file_dict.keys()))
    ...
    def test_result_merging_reads(self):
        ...
        self._assert_directory_formats_equal(obs_reports, exp_reports, type="reports")
        self._assert_directory_formats_equal(obs_outputs, exp_outputs, type="outputs")
